...
SELECT l.language
FROM countrylanguages l
JOIN countries c ON l.countrycode = c.code
...
WHERE l.language = 'Italian' AND l.isofficial = 'T'
GROUP BY c.name
...
FROM cities city
JOIN countries country ON city.countrycode = country.code
WHERE country.name = 'Ecuador' AND city.id = country.capital;
